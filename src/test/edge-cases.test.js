...
describe("Edge Cases and Error Handling", () => {
  ...
  describe("P2PConnectionManager Edge Cases", () => {
    ...
    beforeEach(() => {
      mockPeerConnection = {
        ...
        createAnswer: vi
          .fn()
          .mockResolvedValue({ type: "answer", sdp: "mock" }),
    ...
    it("should handle WebRTC API failures", async () => {
      mockPeerConnection.createOffer.mockRejectedValue(
        new Error("WebRTC not supported")
      );

      await expect(manager.createConnection("peer1", true)).rejects.toThrow(
    ...
    it("should handle sending messages to closed channels", async () => {
      ...
      await expect(
        manager.sendMessage("peer1", { test: "message" })
    ...
    it("should handle large message payloads", async () => {
      ...
      await expect(manager.sendMessage("peer1", largeMessage)).rejects.toThrow(
        "Failed to send message to peer1: Message too large"
      );
    });
  });
  ...
  describe("P2PApp Integration Edge Cases", () => {
    ...
    beforeEach(() => {
      ...
      app = new P2PApp();
      // Note: Mocks are already set up via vi.mock() calls at the top of the file
    });
    ...
    it("should handle signaling connection failures during room join", async () => {
      ...
      await expect(app.joinRoom("test-room")).rejects.toThrow(
    ...
    it("should handle peer connection failures gracefully", async () => {
      ...
      const peerJoinedHandler = mockSignaling.on.mock.calls.find(
    ...
    it("should handle rapid peer join/leave events", async () => {
      ...
      const peerLeftHandler = mockSignaling.on.mock.calls.find(
